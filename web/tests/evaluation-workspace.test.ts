...
function mount(t: { after(callback: () => void): void }, options: { binding?: string; verified?: boolean; delayedHash?: boolean } = {}) {
  ...
  const requests: { url: string; options?: RequestInit; resolve(response: Response): void; reject(reason: Error): void }[] = [];
  ...
  let resolveHash: ((pending: client.EvaluationPending) => void) | undefined, hashInput: client.EvaluationPending | undefined;
  const react = {
  ...
  const dependencies: Record<string, unknown> = { react, "react/jsx-runtime": { jsx, jsxs: jsx }, "next/link": { default: "link" }, "@/components/ui/button": { Button: "button" },
    ...
    "@/components/session-boundary-state": { SESSION_INVALIDATED_EVENT }, "./evaluation-client": { ...client,
      ...(options.delayedHash ? { sealEvaluationAttempt: (pending: client.EvaluationPending) => { hashInput = pending; return new Promise(resolve => { resolveHash = resolve; }); } } : {}) } };
  const module = { exports: {} as { EvaluationWorkspace(props: unknown): Tree } };
  const execute = runInNewContext(`(function(require,module,exports){${compiled}\n})`, { window, document, URLSearchParams, TextDecoder, TextEncoder, crypto,
  ...
  const respond = async (index: number, value: unknown, status = 200, raw = false) => { assert.ok(requests[index], `request ${index}`); requests[index].resolve(new Response(raw ? value as string : JSON.stringify(value), { status })); await flush(); };
  ...
  return { requests, events, window, document, context, render, text, byLabel, button, change, click, submit, fill, ready, respond, session, flush,
    releaseHash: async () => { assert.ok(resolveHash && hashInput); resolveHash(await client.sealEvaluationAttempt(hashInput)); await flush(); },
    reject: async (index: number) => { requests[index].reject(new Error("Synthetic transport loss")); await flush(); },
...
test("lost response then auth 503 hides without deleting exact pending body; manual recovery never automatically POSTs", async t => {
  const f = mount(t); await f.ready(); f.fill(); f.submit(); await f.session(3); const original = f.postRequests()[0].options!.body;
...
test("incorrect save receipt hash/version retains frozen attempt and does not display a verified success", async t => {
  const f = mount(t); await f.ready(); f.fill(); f.submit(); await f.session(3);
  await f.respond(4, { schedule_id: "schedule", version_id: "version", version: 1, schedule_revision: 1, status: "paused", content_hash: "0".repeat(64) });
  assert.match(f.text(), /已保留本页的冻结请求/); assert.doesNotMatch(f.text(), /服务端已返回并校验本次操作回执/); assert.equal(f.postRequests().length, 1);
});
