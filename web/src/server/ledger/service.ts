...
export function revision(db: Database.Database, portfolioId: string): number {
  const row = db.prepare("SELECT revision FROM ledger_heads WHERE portfolio_id=?").get(portfolioId) as { revision: number } | undefined;
...
export function getActiveLedgerEvents(db: Database.Database, portfolioId: string, atRevision = revision(db, portfolioId)): LedgerEventRow[] {
  ...
  const partialCorrection = db.prepare("SELECT id FROM audit_events WHERE portfolio_id=? AND action='correct_ledger' AND json_extract(payload_json,'$.start_revision')<? AND json_extract(payload_json,'$.revision')>? LIMIT 1").get(portfolioId, atRevision, atRevision);
  const partialImport = db.prepare("SELECT id FROM import_batches WHERE portfolio_id=? AND status='confirmed' AND expected_revision<? AND confirmed_revision>? LIMIT 1").get(portfolioId, atRevision, atRevision);
  if (partialCorrection || partialImport) throw new Error("REVISION_NOT_PUBLISHED");
  return db.prepare("SELECT e.* FROM ledger_events e WHERE e.portfolio_id=? AND e.ledger_revision<=? AND e.reversal_of IS NULL AND NOT EXISTS(SELECT 1 FROM ledger_events r WHERE r.reversal_of=e.id AND r.ledger_revision<=?) ORDER BY e.ledger_revision,e.id").all(portfolioId, atRevision, atRevision) as LedgerEventRow[];
}
...
function checkAccount(db: Database.Database, account: string, portfolio: string): void {
  if (!db.prepare("SELECT id FROM accounts WHERE id=? AND portfolio_id=?").get(account, portfolio)) throw new Error("ACCOUNT_OUT_OF_SCOPE");
}
export function validateLedgerTime(command: LedgerCommand, now: string): void {
  ...
  try { new Intl.DateTimeFormat("en", { timeZone: command.source_timezone }); } catch { throw new Error("INVALID_SOURCE_TIMEZONE"); }
  const clock = new Date(now);
  if (!Number.isFinite(clock.getTime())) throw new Error("INVALID_CLOCK");
  if (dateOnly) {
    const parts = new Intl.DateTimeFormat("en-CA", { timeZone: command.source_timezone, year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(clock);
...
export function audit(db: Database.Database, actor: Actor, action: string, objectType: string, objectId: string, portfolio: string | null, rev: number | null, payload: unknown, now: string): string {
  const id = randomUUID();
  db.prepare("INSERT INTO audit_events(id,actor_id,action,object_type,object_id,portfolio_id,ledger_revision,payload_json,created_at) VALUES(?,?,?,?,?,?,?,?,?)").run(id, actor.id, action, objectType, objectId, portfolio, rev, canonical(payload), now);
  return id;
}
...
export function createPortfolio(db: Database.Database, actor: Actor, name: string, now = new Date().toISOString()): string {
  ...
  return db.transaction(() => {
    assertWritableDatabase(db);
    const id = randomUUID();
    db.prepare("INSERT INTO portfolios(id,name,base_currency,created_at) VALUES(?,?,'CNY',?)").run(id, name.trim(), now);
    db.prepare("INSERT INTO ledger_heads(portfolio_id,revision,updated_at) VALUES(?,0,?)").run(id, now);
...
export function createAccount(db: Database.Database, actor: Actor, portfolioId: string, name: string, broker: string, currency: string, now = new Date().toISOString()): string {
  ...
  return db.transaction(() => {
    ...
    db.prepare("INSERT INTO accounts(id,portfolio_id,name,broker,base_currency,status,created_at) VALUES(?,?,?,?,?,'reconciliation_required',?)").run(id, portfolioId, name.trim(), broker.trim(), currency, now);
...
function activeEvent(db: Database.Database, portfolioId: string, eventId: string): LedgerEventRow {
  const event = db.prepare("SELECT e.* FROM ledger_events e WHERE e.id=? AND e.portfolio_id=? AND e.reversal_of IS NULL AND NOT EXISTS(SELECT 1 FROM ledger_events r WHERE r.reversal_of=e.id)").get(eventId, portfolioId) as LedgerEventRow | undefined;
...
export function dividendStateFor(db: Database.Database, portfolioId: string, rootEventId: string): DividendState {
  ...
  const children = db.prepare("SELECT e.* FROM ledger_events e WHERE e.portfolio_id=? AND json_extract(e.payload_json,'$.fact.related_event_id')=? AND e.reversal_of IS NULL AND NOT EXISTS(SELECT 1 FROM ledger_events r WHERE r.reversal_of=e.id) ORDER BY e.ledger_revision,e.id").all(portfolioId, rootEventId) as LedgerEventRow[];
...
function stateFor(db: Database.Database, command: LedgerCommand, context: RecordContext): FactState {
  ...
  if (fact.listing_id) {
    ...
    const pos = db.prepare("SELECT quantity,cost_amount,cost_known,currency FROM position_projections WHERE account_id=? AND listing_id=?").get(fact.account_id, fact.listing_id) as (Omit<Position, "cost_known"> & { cost_known: number }) | undefined;
    if (pos) state.position = { ...pos, cost_known: Boolean(pos.cost_known) };
    if (fact.type === "split") state.transit_positions = (db.prepare("SELECT * FROM security_transit_projections WHERE source_account_id=? AND listing_id=?").all(fact.account_id, fact.listing_id) as (Omit<TransitPosition, "cost_known"> & { cost_known: number })[])
  ...
  if (fact.related_event_id) {
    ...
    if (["security_transfer_in", "security_transfer_return"].includes(fact.type)) {
      ...
      const lot = db.prepare("SELECT * FROM security_transit_projections WHERE transfer_event_id=?").get(source.id) as (Omit<TransitPosition, "cost_known"> & { cost_known: number }) | undefined;
    ...
    const rows = db.prepare("SELECT amount FROM postings WHERE event_id=? AND ledger_account=?").all(source.id, ledgerAccount) as { amount: string }[];
    let outstanding = rows.reduce((s, r) => s.add(amount(r.amount)), new Decimal(0));
    if (original.type === "buy") outstanding = outstanding.neg();
    const children = db.prepare("SELECT e.payload_json FROM ledger_events e WHERE e.portfolio_id=? AND json_extract(e.payload_json,'$.fact.related_event_id')=? AND e.reversal_of IS NULL AND NOT EXISTS(SELECT 1 FROM ledger_events r WHERE r.reversal_of=e.id)").all(command.portfolio_id, source.id) as { payload_json: string }[];
    for (const child of children) {
  ...
  if (fact.type === "dividend" && fact.tax === undefined && context.correctionId && context.supersedesEventId) {
    const prior = db.prepare("SELECT payload_json FROM ledger_events WHERE id=? AND portfolio_id=? AND account_id=? AND event_type='dividend'").get(context.supersedesEventId, command.portfolio_id, fact.account_id) as { payload_json: string } | undefined;
...
function applyProjection(db: Database.Database, entry: Entry, rev: number): void {
  for (const p of entry.postings) {
    const old = db.prepare("SELECT balance FROM account_projections WHERE account_id=? AND currency=? AND ledger_account=?").get(p.account_id, p.currency, p.ledger_account) as { balance: string } | undefined;
    const balance = exact(amount(old?.balance ?? "0").add(amount(p.amount)));
    db.prepare("INSERT INTO account_projections(account_id,currency,ledger_account,balance,ledger_revision) VALUES(?,?,?,?,?) ON CONFLICT(account_id,currency,ledger_account) DO UPDATE SET balance=excluded.balance,ledger_revision=excluded.ledger_revision").run(p.account_id, p.currency, p.ledger_account, balance, rev);
  }
  for (const m of entry.movements) {
    const old = db.prepare("SELECT quantity,cost_amount,cost_known FROM position_projections WHERE account_id=? AND listing_id=?").get(m.account_id, m.listing_id) as { quantity: string; cost_amount: string; cost_known: number } | undefined;
...
export function invalidateAccounts(db: Database.Database, accountIds: Iterable<string>): void {
  const update = db.prepare("UPDATE accounts SET status='reconciliation_required',row_version=row_version+1 WHERE id=? AND status<>'disabled'");
...
function duplicateReceipt(db: Database.Database, receipt: Receipt): Receipt {
  const superseded = db.prepare("SELECT id FROM ledger_events WHERE reversal_of=?").get(receipt.event_id);
...
export function resolveSourceReceipt(db: Database.Database, command: LedgerCommand): Receipt | undefined {
  ...
  const alias = db.prepare(`SELECT r.normalized_json,o.result_json FROM import_rows r
  ...
  const source = db.prepare("SELECT id,payload_hash,payload_json,ledger_revision FROM ledger_events WHERE account_id=? AND source_id=? AND source_event_id=? AND event_type=?").get(command.fact.account_id, command.source_id, command.source_event_id, command.fact.type) as StoredEvent | undefined;
  if (!source) return;
  if (sourceFingerprint(JSON.parse(source.payload_json)) !== sourceFingerprint(command)) throw new Error("SOURCE_DUPLICATE_CONFLICT");
  const auditRow = db.prepare("SELECT id,payload_json FROM audit_events WHERE object_id=? AND action='record_fact'").get(source.id) as { id: string; payload_json: string };
...
export function recordFact(db: Database.Database, actor: Actor, command: LedgerCommand, now = new Date().toISOString(), context: RecordContext = {}): Receipt {
  ...
  return db.transaction(() => {
    ...
    if (context.importBatchId && !db.prepare("SELECT id FROM import_batches WHERE id=? AND portfolio_id=? AND account_id=? AND attachment_id IS NOT NULL AND status IN ('preview','confirmed')").get(context.importBatchId, command.portfolio_id, command.fact.account_id)) throw new Error("IMPORT_BATCH_OUT_OF_SCOPE");
    ...
    const duplicate = db.prepare("SELECT payload_hash,result_json FROM command_dedup WHERE scope=? AND idempotency_key=?").get(scope, command.idempotency_key) as { payload_hash: string; result_json: string } | undefined;
    if (duplicate) {
    ...
    if (sourceReceipt) {
      db.prepare("INSERT INTO command_dedup(scope,idempotency_key,payload_hash,result_json,created_at) VALUES(?,?,?,?,?)").run(scope, command.idempotency_key, digest, canonical(sourceReceipt), now);
      assertWritableDatabase(db);
    ...
    if (command.fact.type.startsWith("opening_")) {
      const openings = db.prepare("SELECT e.event_type,e.payload_json,e.effective_at FROM ledger_events e WHERE e.account_id=? AND e.event_type IN ('opening_cash','opening_position') AND e.reversal_of IS NULL AND NOT EXISTS(SELECT 1 FROM ledger_events r WHERE r.reversal_of=e.id)").all(command.fact.account_id) as { event_type: string; payload_json: string; effective_at: string }[];
    ...
    const transit = db.prepare("INSERT INTO security_transit_movements(id,event_id,transfer_event_id,source_account_id,target_account_id,listing_id,currency,quantity,cost_amount,cost_known) VALUES(?,?,?,?,?,?,?,?,?,?)");
    ...
    db.prepare("UPDATE ledger_heads SET revision=?,updated_at=? WHERE portfolio_id=? AND revision=?").run(rev, now, command.portfolio_id, previous);
    ...
    db.prepare("INSERT INTO command_dedup(scope,idempotency_key,payload_hash,result_json,created_at) VALUES(?,?,?,?,?)").run(scope, command.idempotency_key, digest, canonical(receipt), now);
    assertWritableDatabase(db);
    return receipt;
  }).immediate();
}
...
export function rebuildProjections(db: Database.Database, actor: Actor, portfolioId: string, now = new Date().toISOString()): void {
  ...
  db.transaction(() => {
    ...
    db.prepare("DELETE FROM account_projections WHERE account_id IN (SELECT id FROM accounts WHERE portfolio_id=?)").run(portfolioId);
    db.prepare("DELETE FROM position_projections WHERE account_id IN (SELECT id FROM accounts WHERE portfolio_id=?)").run(portfolioId);
    db.prepare("DELETE FROM security_transit_projections WHERE source_account_id IN (SELECT id FROM accounts WHERE portfolio_id=?)").run(portfolioId);
    const events = getActiveLedgerEvents(db, portfolioId);
    for (const event of events) {
      const postings = db.prepare("SELECT account_id,currency,ledger_account,amount FROM postings WHERE event_id=? ORDER BY id").all(event.id) as Entry["postings"];
      const rows = db.prepare("SELECT account_id,listing_id,currency,quantity,cost_amount,cost_known FROM position_movements WHERE event_id=? ORDER BY id").all(event.id) as (Omit<Entry["movements"][number], "cost_known"> & { cost_known: number })[];
      const transits = db.prepare("SELECT transfer_event_id,source_account_id,target_account_id,listing_id,currency,quantity,cost_amount,cost_known FROM security_transit_movements WHERE event_id=? ORDER BY id").all(event.id) as (Omit<TransitPosition, "cost_known"> & { cost_known: number })[];
