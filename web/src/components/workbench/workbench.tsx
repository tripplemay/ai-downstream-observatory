...
export function Workbench({ initial }: { initial: WorkbenchState }) {
  ...
  async function refresh(portfolio = selected) {
  ...
  return <div inert={!hydrated} aria-busy={busy} data-hydrated={hydrated} className="mx-auto flex w-full max-w-7xl flex-col gap-6 p-4 md:p-8">
    ...
    <Section title="组合工作区"><div className="flex flex-wrap gap-3">{state.portfolios.map(p => <Button key={p.id} variant={selected === p.id ? "default" : "outline"} disabled={busy} onClick={() => void perform(async () => { setPreview(null); await refresh(p.id); })}>{p.name}</Button>)}</div><form className="mt-4 flex flex-wrap items-end gap-3" onSubmit={e => submit(e, async data => { const result = await mutate({ action: "create_portfolio", name: data.get("name") }); await refresh(result.id); setNotice("组合已创建。尚无资金计划和现金；需另行录入并确认。"); })}><label className={field}>新组合名称<Input name="name" maxLength={120} required placeholder="长期 ETF 组合" /></label><Button disabled={busy}>建立空账本</Button></form></Section>
    {selected && <>
      ...
      <fieldset disabled={state.read_only || busy} className="min-w-0 space-y-3"><label className={field}>CSV 导入账户<select aria-label="CSV 导入账户" className={selectClass} value={selectedCsvAccount} onChange={event => setCsvAccount(event.target.value)}>{state.accounts.map(account => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><CsvWorkspace portfolioId={selected} accountId={selectedCsvAccount} revision={state.revision} onCommitted={() => { void perform(async () => { await refresh(); setPreview(null); }); }} /></fieldset>
