...
import { CsvBackgroundWorkspace } from "./csv-background-workspace";
...
export function Workbench({ initial }: { initial: WorkbenchState }) {
  ...
  const [legacyCsvRecovery, setLegacyCsvRecovery] = useState(false);
  ...
  return <div inert={!hydrated} aria-busy={busy} data-hydrated={hydrated} className="mx-auto flex w-full max-w-7xl flex-col gap-6 p-4 md:p-8">
    ...
    {selected && <>
      ...
      <fieldset disabled={busy} className="min-w-0 space-y-3"><label className={field}>CSV 导入账户<select aria-label="CSV 导入账户" className={selectClass} value={selectedCsvAccount} disabled={csvScopeLocked} onChange={event => { if (!csvScopeLocked) setCsvAccount(event.target.value); }}>{state.accounts.map(account => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label>
        <Button type="button" variant="outline" disabled={csvScopeLocked} onClick={() => { if (!csvScopeLocked) setLegacyCsvRecovery(value => !value); }}>{legacyCsvRecovery ? "返回后台 CSV 主流程" : "仅恢复旧版 CSV 确认记录"}</Button>
        {legacyCsvRecovery ? <CsvWorkspace legacyRecoveryOnly portfolioId={selected} accountId={selectedCsvAccount} revision={state.revision} readOnly={state.read_only} onScopeLockChange={setCsvScopeLocked} onRestorePendingScope={restoreCsvScope} onCommitted={csvCommitted} />
          : <CsvBackgroundWorkspace portfolioId={selected} accountId={selectedCsvAccount} revision={state.revision} readOnly={state.read_only} onScopeLockChange={setCsvScopeLocked} onRestorePendingScope={restoreCsvScope} onCommitted={csvCommitted} />}
      </fieldset>
