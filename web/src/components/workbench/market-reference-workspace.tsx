...
export function MarketReferenceWorkspace({ initial, initialSessionBinding }: { initial: MarketReferenceState; initialSessionBinding: string }) {
  ...
  return <main className="mx-auto max-w-6xl space-y-5 p-4 md:p-6">
    <header className={panel}><h1 className="text-2xl font-semibold">市场资料与价格采集</h1><p className="text-sm text-muted-foreground">来源存储不是审核；人工映射和日历审核不是 provider 认证、账户可买权限或策略准入。SDK 投影不是网络原始响应，收市时间不是报价发布时间。不写交易或资金事实。</p><Link href="/workbench" target="_blank" rel="noopener noreferrer" className="text-sm underline">在新页核对工作台与任务结果</Link><p className="text-xs text-muted-foreground">未决请求仅保存在当前页内存，不是持久恢复记录；强制关闭、刷新或离开可能丢失原请求。</p></header>
