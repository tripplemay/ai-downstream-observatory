...
原 `research-plan-v1` / `research-dataset-v1` 保留仅新增资金的固定权重模型。
新 v2 计划与数据显式配对，支持月度动量/均线轮动、卖出和分阶段交收，以及同执行引擎的固定再平衡基准。
完整参数、公司行动信号口径、理论除权 carry、固定数量和现金依赖见 [轮动研究契约](../../docs/rotation-research.md)。
v2 不继承个人资金默认值、实盘权限或前向观察资格，也不把旧报告改写为新算法。
...
两个版本的三个研究窗口均从初始资金重新开始，**不是连续的前向持仓历史**。v1 没有卖出；v2 的卖出、股票和现金交收使用显式研究日历。两者均不支持杠杆、现金利息、自动汇回外币股息、实盘结算引擎或真实时间前向跟踪。佣金和税覆盖均为输入假设，尚未完成券商、账户可买性及完整税务核验。
