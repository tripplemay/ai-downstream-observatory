...
SIGNALS = [
    ...
    ("C11", "platform", "AI 原生入口分发力", "ChatGPT/Gemini 等 AI 原生入口的用户规模与增速；"
     "传统搜索/分发入口被 AI 答案侵蚀的速度（谷歌核心风险）",
     "SimilarWeb 等公开数据、各公司公告；AI 季度核对",
     "AI 原生入口持续高增且开始承接商业化闭环（广告/交易/订阅之外的变现）→ 平台归属判断需修正",
     "—", "未验证", "2026-08-02", ""),
...
POOL = [
    ("易方达中概互联网ETF及联接", "513050（场外联接更优）", "QDII", "中国平台，腾讯+阿里约半仓",
     "主仓：中国平台纯度最高，其余成分弱相关但可接受；场内常溢价，长期建仓走场外"),
    ("天弘中美互联网", "009225", "QDII 场外", "中美两边互联网平台",
     "卫星：一篮子简化决策，但两端纯度都稀释；规模小，注意流动性"),
    ("纳指科技ETF及联接", "159509", "QDII", "美股平台，纯度最高",
     "美股平台主选；溢价风险同上"),
    ("纳指100ETF及联接", "513100 / 159941", "QDII", "美股平台，更均衡",
     "美股平台次选：F1 敏感期（capex 军备竞赛）的降波动选择"),
    ("博时标普500ETF及联接", "513500", "QDII", "更宽的底仓选择",
     "防御底仓：thesis 纯度最低、波动最小；科技权重约三成"),
    ("恒生科技/恒生互联网", "513180 / 513330", "QDII", "港股平台",
     "与主仓同源：仅作 QDII 额度受限时的替代通道，不与主仓并行加；波动大于中概互联"),
    ("货币基金/短债ETF", "511880（或同类）", "场内", "等待期资金停泊",
     "等待期（红灯/黄灯未确认）资金停泊，不打无收益等待；非 thesis 标的"),
]
...
- 通用长期研究示例：依据证实/证伪信号而非短期波动调整研究判断，不代表任何用户的个人投资期限或偏好。"""
