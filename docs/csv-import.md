...
未决请求仅保存在当前页面内存；普通链接点击及整页刷新/关闭有提示，但 Next.js 同文档浏览器后退/前进可能不触发这些提示，强制离开后也不保证恢复。持久化恢复与完整导航故障验收尚未完成，请先核对服务器批次状态再离开。下载原件与映射用于独立核对，不等于已完成账户对账。向导的原生桌面/窄屏/无障碍及中断故障交互验收仍须单独完成，服务测试不替代浏览器验收。
