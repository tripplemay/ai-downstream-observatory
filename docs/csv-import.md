...
状态：已接入原始文件上传、不可变映射版本、逐行预览/重复核对、原子确认及高级 Web 界面；属于通用显式映射，不是券商原生适配器。具体券商须提供获授权的脱敏样本并单独核验，不宣称已经支持或认证其原生格式；D-07、ACC-18 和 P-02 的真实样本验收仍未闭环。
...
高级 UI 入口在账户账本，需加载或编辑完整映射 JSON；尚不是表头拖拽向导。每页 25 行、每组候选 20 项，跨页保留全部决定；没有自动勾选或默认重复判定。范围/版本变化清除旧确认，确认响应失败保留同一序列化载荷供重试，并可查询服务器状态。下载原件与映射用于独立核对，不等于已完成账户对账。
...
10,000 行解析及 50,000 条事实候选索引均为合成边界测试，不代表 10,000 行数据库确认的 60 秒目标。仍需后台导入任务、大批量确认性能/故障测试、可视化映射向导，以及真实券商去标识样本的费用/结算/撤单/部分成交/编码和最终账户对账验收。当前不支持新增五类证券实物/在途事件的 CSV 映射，也不预填真实账户事实。
